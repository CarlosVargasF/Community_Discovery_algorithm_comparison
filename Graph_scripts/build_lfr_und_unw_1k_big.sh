...
do
	for iter in $(seq 0 9)	
	do
		$script_full_path/lfrbench_udwov.exe -N 1000 -k 20 -maxk 50 -minc 20 -maxc 100 -muw ${mu} -name lfr_ut_${mu}_rep_${iter} -beta 1 -cnl 1 
	done
done

#Moving output files to corresponding sub-folder

dest_path="../LFR_benchmarks_for_testing/undirected_unweighted/lfr_n_1k_sz_big"
